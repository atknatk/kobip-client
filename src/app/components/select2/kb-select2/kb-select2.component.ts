...
import { KbSelect2Item } from './kb-select2-item';
...
const VALUE_ACCESSOR = {
  ...
  useExisting: forwardRef(() => KbSelect2Component),
  multi: true
};
const noop = () => {
};
...
@Component({
  selector: 'kb-select2',
  templateUrl: './kb-select2.component.html',
  styleUrls: ['./kb-select2.component.css'],
  providers: [VALUE_ACCESSOR]
})
export class KbSelect2Component implements AfterViewInit, ControlValueAccessor {

  MORE_RESULTS_MSG = 'Showing ' + Messages.PARTIAL_COUNT_VAR + ' of ' +
    Messages.TOTAL_COUNT_VAR + ' results. Refine your search to show more results.';
  ...
  @Input() kbSelect2ItemAdapter: (entity: any) => KbSelect2Item;
  ...
  @Output() onSelect: EventEmitter<KbSelect2Item> = new EventEmitter<KbSelect2Item>();
  @Output() onRemove: EventEmitter<KbSelect2Item> = new EventEmitter<KbSelect2Item>();
  @ViewChild('results') results: KbSelect2ResultsComponent;
  templateRef: TemplateRef<any>;
  term = new FormControl();
  resultsVisible = false;
  listData: KbSelect2Item[];
  fullDataList: KbSelect2Item[];
  selectedItems: KbSelect2Item[] = [];
  searchFocused = false;
  isLoading = false;
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => this.dataSourceSubscription.closed = true, 0);
  ...
  private adaptItems(items: any[]): KbSelect2Item[] {
    const convertedItems = [];
    items.map((item) => this.kbSelect2ItemAdapter(item))
      .forEach((KbSelect2Item) => convertedItems.push(KbSelect2Item));
    return convertedItems;
  }

  private alreadySelected(item: KbSelect2Item): boolean {
    let result = false;
  ...
  private fetchAndfilterLocalData(term: string): Observable<KbSelect2Item[]> {
    if (!this.fullDataList) {
      return this.fetchData('').pipe(
        mergeMap((items) => {
          this.fullDataList = items as KbSelect2Item[];
  ...
  private fetchData(term: string): Observable<KbSelect2Item[]> {
    return this
      .dataSourceProvider(term, this.buildValue())
      .pipe(map((items: any[]) => this.adaptItems(items)));
  }

  private filterLocalData(term: string): Observable<KbSelect2Item[]> {
  ...
  private handleMultipleWithEntities(selectedValues: any) {
    ...
    selectedValues.forEach((entity) => {
      const item = this.kbSelect2ItemAdapter(entity);
  ...
  private handleMultipleWithIds(selectedValues: any) {
    if (selectedValues !== undefined && this.selectedProvider !== undefined) {
      ...
      this.selectedProvider(uniqueIds).subscribe((items: any[]) => {
        this.selectedItems = items.map(this.kbSelect2ItemAdapter);
      });
    }
  }
  ...
  private handleSingleWithId(id: any) {
    if (id !== undefined && this.selectedProvider !== undefined) {
      this.selectedProvider([id]).subscribe((items: any[]) => {
        items.forEach((item) => {
          const KbSelect2Item = this.kbSelect2ItemAdapter(item);
          this.selectedItems = [KbSelect2Item];
          this.placeholderSelected = KbSelect2Item.text;
        });
      });
    }
  }

  private loadDataFromObservable(term: string): Observable<KbSelect2Item[]> {
  ...
  private populateItemsFromEntities(selectedValues: any) {
    if (this.multiple) {
      ...
      const KbSelect2Item = this.kbSelect2ItemAdapter(selectedValues);
      this.selectedItems = [KbSelect2Item];
      this.placeholderSelected = KbSelect2Item.text;
    }
  }
  ...
  private subscribeToResults(observable: Observable<string>): void {
    this.dataSourceSubscription = observable.pipe(
      tap(() => this.resultsVisible = false),
      filter((term: any) => term.length >= this.minimumInputLength),
      switchMap((term: any) => this.loadDataFromObservable(term)),
      map((items: any) => items.filter(item => !(this.multiple && this.alreadySelected(item)))),
